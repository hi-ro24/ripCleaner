...
def delete_matching_files(rip_name, path, log_dir):
    ...
    for filename in os.listdir(path):
        if is_valid_tiff(filename):
            ...
            # 隍・焚縺ｮ繝√ぉ繝・け繧・縺､縺ｮ髢｢謨ｰ縺ｫ縺ｾ縺ｨ繧√ｋ
            if not is_file_ready_for_deletion(full_path):
                print(f"[{rip_name}] Skipped (Not ready): {filename}")
                skipped_files.append((filename, "Not ready"))
                continue

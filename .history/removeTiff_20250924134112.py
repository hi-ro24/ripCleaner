    ...
    for filename in os.listdir(path):
        if is_valid_tiff(filename):
            ...
            if is_file_locked(full_path):
                print(f"[{rip_name}] Skipped (Locked): {filename}")
                skipped_files.append((filename, "Locked"))
                continue
                
            if not is_file_complete(full_path):
                print(f"[{rip_name}] Skipped (Incomplete): {filename}")
                skipped_files.append((filename, "Incomplete"))
                continue
            ...
            try:
                ...
                print(f"[{rip_name}] Delete error: {filename} 竊・{e}")
        ...
        if filename.endswith('.log'):
            ...
            if (current_time - file_time).days > days_to_keep:
                try:
                    ...
                    print(f"蜿､縺・Ο繧ｰ縺ｮ蜑企勁縺ｫ螟ｱ謨・ {filename} 竊・{e}")
...
def main():
    ...
    print(f"ripCleaner version {VERSION} 襍ｷ蜍輔＠縺ｾ縺励◆縲・)
...
if __name__ == "__main__":
    try:
        ...
        print(f"莠域悄縺帙〓繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆: {e}")
        sys.exit(1)

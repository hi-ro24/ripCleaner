...
def delete_matching_files(rip_name, path, log_dir):
    ...
    for filename in os.listdir(path):
        if is_valid_tiff(filename):
            ...
            try:
                if delete_with_retry(full_path):  # 繝ｪ繝医Λ繧､讖溯・繧剃ｽｿ逕ｨ
                    deleted_files.append(filename)
                    print(f"[{rip_name}] 蜑企勁: {filename}")
                else:
                    print(f"[{rip_name}] 蜑企勁螟ｱ謨暦ｼ医Μ繝医Λ繧､蠕鯉ｼ・ {filename}")
            except Exception as e:
                print(f"[{rip_name}] 蜑企勁繧ｨ繝ｩ繝ｼ: {filename} 竊・{e}")
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] == "--version":
        print(f"ripCleaner version {VERSION}")  # 繝励Ο繧ｰ繝ｩ繝蜷阪ｒ菫ｮ豁｣
        return

    print(f"ripCleaner version {VERSION} 襍ｷ蜍輔＠縺ｾ縺励◆縲・)  # 繝励Ο繧ｰ繝ｩ繝蜷阪ｒ菫ｮ豁｣
